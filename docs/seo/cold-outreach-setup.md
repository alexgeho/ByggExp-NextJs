...
**Где остановились:** база собрана (924 email), домен готов, тест отправлен но НЕ проверен (inbox/spam).
Дальше: проверить тест → прогрев домена (1–2 нед) → чистка+сегментация базы (бесплатно) → запуск 20–30/день.
...
## Источники данных / сегментация по размеру (исследовано 2026-09-02)

Задача: сегментировать базу по числу сотрудников (ICP = 2–20 чел), в идеале с email + авто-выгрузка.

- **allabolag.se/segmentering** — фильтры (bransch, plats, **Anställda**, omsättning, ✓E-post) БЕСПЛАТНО,
  но: превью только ~10 фирм, пагинации нет, **выгрузка платная** — «Exportera till Excel» =
  **1990 kr** (≤5000 фирм) или **Allabolag Plus** (подписка, 100/мес). Бесплатного API нет.
  - Экспорт-колонки (проверено на примере `Allabolag_lista.xlsx`): BOLAGSNAMN, ORG.NR, **E-POST**,
    TELEFON, **HEMSIDA**, **ANTAL ANSTÄLLDA**, OMSÄTTNING, **ORDFÖRANDE/VD (ЛПР)**, ADRESS, SNI.
  - ⚠️ **E-POST заполнен НЕ у всех** (в примере 3 из 11 — у гигантов почты в реестре нет).
    Поэтому: экспортить БЕЗ галки «E-post» (чтобы получить HEMSIDA всех) → недостающие почты
    добить моим скрейпером с сайтов. allabolag = реестр → активность/банкроты видны, + имя ЛПР.
- **Платные альтернативы:** Prospektly.se (🇸🇪, free tier + API, фильтр anställda), CompanyData.com
  (1.85M шв. фирм, verified email, CSV/API), Goava (Nordic), Apollo.io (глобал, verified email +
  API, но мелкие шв. трейды покрыты слабо).
- **Бесплатного «всё автоматом с email» НЕТ.** Самое полное для мелких el/bygg = реестр + скрейп сайтов.
- **РЕШЕНИЕ (бесплатный путь):** взять наши 924 (уже с email) + **обогатить `antal_anställda`
  с бесплатных страниц allabolag** → сегментация по размеру за €0. (Риск лимитов allabolag — батчами.)

## Решения по письму
- **Ссылку в ПЕРВОМ письме НЕ ставим** (спам-триггер, особенно с непрогретого домена). Сейчас в письме
  ссылки нет — это правильно. Сайт `byggexp.se` — либо строкой в подпись, либо во втором касании (на ответ).

## Следующие шаги ⏭️

**Блок «база»:**
1. [x] ✅ Собрано 924 email с сайтов → `~/Downloads/byggexp-emails-final.csv` (скрипт scrape-emails.mjs).
2. [ ] Обогатить `antal_anställda` (бесплатно с страниц allabolag) → сегмент 2–20 чел = ICP.
3. [ ] Исключить гигантов не-ICP (Bravida, Veteranpoolen, крупные сети) — заодно уйдут по фильтру размера.
4. [ ] Спот-чек банкротов по allabolag (хотя бы топ-адресатов).
